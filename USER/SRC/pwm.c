 ...
 * @LastEditors: 叮咚蛋
 * @LastEditTime: 2020-11-09 20:33:35
 * @FilePath: \F4舵机板\USER\SRC\pwm.c
 */
#include "pwm.h"
...
void PWM_Init(u16 arr, u16 psc)
{
	...
	TIM_OC2Init(TIM4, &TIM_OCInitStructure); //PB7,初始化外设TIM4-CH2
...
void pwm_out(struct Engine_Datadef *engine)
{
	...
	case PB4:
		...
		if (engine->Move_Flag == 1)
		{
			TIM_SetCompare1(TIM3, engine->Move_Ratio * temp.Engine_PWM_arr);
		}
		break;
	...
	case PB7:
		...
		if (engine->Move_Flag == 1)
		{
			TIM_SetCompare2(TIM4, engine->Move_Ratio * temp.Engine_PWM_arr);
		}
		break;
	default:
		break;
